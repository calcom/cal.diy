...
import { Calendar as CalendarIcon, ChevronRight, ListIcon, Refresh } from "../Icons";
...
export function TeamApp() {
  ...
  const list = (
    <div style={{ display: "grid", gap: 4 }}>
      {loading &&
        [0, 1, 2].map((i) => <div key={i} className="shimmer" style={{ height: 64, marginBottom: 6 }} />)}
        ...
        <div key={key} className="window-day rise">
          ...
          {items.map((w) => (
            <button
              ...
              <span className="bar" />
              <span>
                <strong>{formatRange(new Date(w.start), new Date(w.end))}</strong>
                <small>{formatDuration((w.end - w.start) / MINUTE)} open</small>
              </span>
              <span className="icon-btn sm go">
    ...
    <div className="shell">
      ...
      <div className="workspace">
        ...
        <main className="main-panel">
          ...
          {mobileMode === "list" ? (
            <section className="card card-pad mobile-only" style={{ flex: 1, overflowY: "auto" }}>
              <DurationPicker value={duration} onChange={setDuration} />
              <div style={{ marginTop: 12 }}>{list}</div>
            </section>
          ) : null}
...
function TeamHero({ totalMinutes, compact }: { totalMinutes: number; compact?: boolean }) {
  ...
  return (
    <section className="card card-sage card-pad team-hero">
      <p className="eyebrow">Team booking</p>
      <h1 style={compact ? { fontSize: 26 } : undefined}>
        Grab time with <b style={{ fontWeight: 600 }}>Z</b> &amp; <b style={{ fontWeight: 600 }}>XOE</b>
      </h1>
      {!compact && (
        <p>
          Only Z's free time shows here. Pick a window, choose a length, and the call lands straight in both
          calendars with a Meet link.
        </p>
      )}
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: compact ? 8 : 16 }}>
        <div className="avatars" style={{ marginTop: 0 }}>
          <span className="avatar" style={{ background: "#7B75C9" }}>
            Z
          </span>
          <span className="avatar" style={{ background: "#1F2322" }}>
            X
          </span>
        </div>
        <div className="stat">
          <span className="stat-value" style={{ fontSize: compact ? 26 : 34 }}>
