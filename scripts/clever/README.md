...
|---|---|---|
| `CC_PRUNE_DEV_DEPENDENCIES` | `false` | `yarn workspaces focus --all --production` after the build — drops ~2 GB of devDependencies. Requires a run command that does not go through `turbo` (e.g. `yarn workspace @calcom/web start`). |
